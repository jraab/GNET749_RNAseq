...
# There is a current issue with RSqlite affecting loading of some packages
...
# we can use bioMart to look these up and add to our results frame
...
load('/Users/jraab/GitHub/GNET749_RNAseq/data/GSE102560_dds.Rda')# load dds
...
kegg_up <- enrichKEGG(gene         = brg1_up$entrezgene_id, 
                      organism     = 'hsa',
                      pvalueCutoff = 0.05)

kegg_down <- enrichKEGG(gene   = brg1_down$entrezgene_id, 
                        organism = 'hsa', 
                        pvalueCutoff = 0.05)
dotplot(kegg_up) 
barplot(kegg_up)
...
brg_up_go <- enrichGO(gene    = brg1_up$entrezgene_id,
...
brg_down_go <- enrichGO(gene          = brg1_down$entrezgene_id,
                      universe      = brg_res$entrezgene_id,
                      OrgDb         = org.Hs.eg.db,
                      ont           = 'MF',
                      pAdjustMethod = "BH",
                      ...
                      readable      = TRUE)
dotplot(brg_down_go)
dotplot(simplify(x = brg_down_go, by = 'p.adjust', cutoff = 0.7) )
...
brg1_up_enr <- enricher(gene = brg1_up$rowname, TERM2GENE = msigdbr_t2g)
brg1_down_enr <- enricher(gene = brg1_down$rowname, TERM2GENE = msigdbr_t2g)
dotplot(brg1_up_enr)
